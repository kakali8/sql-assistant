-- metric_id: pk_click_to_enter
-- metric_name: PK进入率
-- card_name: PK click to enter
-- card_id: 3231
-- dashboard: L2 (id=522)
-- business_domain: PK
-- owner: ops, product
-- definition: PK 请求 → 成功进入
-- description: PK 请求成功进入的比例
...
SELECT
    ...
    countDistinctIf((m2.pk_id, m2.acc_pid), (m2.last_status IN ('accepted','ready','fighting','gameover'))
        OR (m2.stage_final_status IN ('accepted','ready','fighting','gameover'))) AS numerator,
    ...
    --     OR (m2.stage_final_status IN ('ready','fighting','gameover'))) AS ready_pk_pv,
