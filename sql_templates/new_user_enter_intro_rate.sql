-- metric_id: new_user_enter_intro_rate
-- metric_name: 新用户进入介绍页漏斗
-- card_name: New user enter intro rate
-- card_id: 3285
-- dashboard: CEO (id=518)
-- business_domain: User Growth
-- owner: marketing, product
-- definition: 新用户进入介绍页率
...
NULLIF(count(distinct acc_pid),0) as denominator,
NULLIF(count(distinct case when event_name in ('new_user_intro_view') and type = 1 then t2.device_id else null end),0) as numerator,
...
LEFT JOIN hab_events AS h1
    ON t1.acc_pid = h1.device_id
   AND h1.biz_date = t1.acc_date + 1
