-- metric_id: kix_reward_uv
-- metric_name: KiX challenge奖励人数
-- card_name: KiX reward uv
-- card_id: 3201
-- dashboard: CEO (id=518)
-- business_domain: Daily Challenge
-- owner: ops, marketing
-- definition: 每日KiX challenge奖励人数
...
reward_user_day AS (
    ...
    WHERE reward_type = '$'
	and s1.pk_id in (
    select pk_id from rings_broadcast.pk_activity where  activity_info_id is not null and HOUR(FROM_UNIXTIME(activity_start_time / 1000)) = 13
	)
	and reward>0
...
LEFT JOIN redeem_day  c
   ...
   and r.pk_id=c.pk_id
left join loops_game.game_version_info as s1
