-- metric_id: new_user_daily_tournament_d1_retention
-- metric_name: 新用户Daily Tournament留存
-- card_name: New user daily tournament D1 retention
-- card_id: 3368
-- dashboard: CEO (id=518)
-- business_domain: Daily tournament
-- owner: ceo, marketing, product, ops
-- definition: 每天Daily Tournament新用户隔日是否继续参赛
-- description: 关注新用户在Daily Tournament中的留存情况
...
SELECT
   ...
   COALESCE(
    ...
    countDistinct(b1.acc_pid) AS denominator,
	countDistinct(b2.acc_pid) AS numerator,
