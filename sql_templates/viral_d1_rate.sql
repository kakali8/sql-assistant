-- metric_id: viral_d1_rate
-- metric_name: viral来源用户的留存
-- card_name: Viral D1 rate
-- card_id: 3219
-- dashboard: CEO (id=518)
-- business_domain: Viral
-- owner: ops, product
-- definition: 指标定义
-- description: viral来源用户的留存
...
SELECT
	...
	ifnull(COUNT(DISTINCT h1.user_id),0) as numerator,
	ifnull(COUNT(DISTINCT s1.to_uid),0) as denominator,
    ifnull(
        1.0 * COUNT(DISTINCT h1.user_id)
        / NULLIF(COUNT(DISTINCT s1.to_uid), 0),
        0
    )                                                             AS value
